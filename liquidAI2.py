...
BRAIN_FILE = "ai_brain_liquid_main.joblib"
SIGNALS_FOLDER = "Signals_Liquid2"

if not os.path.exists(SIGNALS_FOLDER):
    os.makedirs(SIGNALS_FOLDER)

# --- PROVEN PARAMETERS ---
MIN_PROBABILITY = 0.55  # Lowered to capture more liquid opportunities
...
FEATURES = [
    # --- Momentum & Trend ---
    'RSI', 'Trend_Score', 'MACD', 'MACD_Signal', 'MACD_Hist', 'BB_Score',
    
    # --- Volatility & Scaling ---
    'Volatility', 'ATR', 'ATR_Ratio', 
    
    # --- Liquidity & Institutional Flow ---
    'Vol_Force', 'VWAP_Score', 'Volume_Trend',
    
    # --- Market Context (Critical for Liquid Stocks) ---
    'Relative_Performance', 'Index_Momentum', 'Market_Regime_Score',
...
def prepare_features(df, fund_df):
    ...
    df['ATR_Ratio'] = df['ATR'] / df['close'].replace(0, 1e-9)
    
    # Volume Trend
    df['Volume_Trend'] = df['volume'].pct_change(5)
    
    # Market Regime Score (based on index performance)
    if INDEX_DF is not None:
        idx_sma_50 = INDEX_DF['close'].rolling(50).mean()
        market_regime = (INDEX_DF['close'] - idx_sma_50) / idx_sma_50.replace(0, 1e-9)
        df = df.join(market_regime.to_frame('Market_Regime_Score'), how='left')
    else:
        df['Market_Regime_Score'] = 0
    
    # ---------------------------
    # IMPROVEMENT: Add Index Momentum as a feature
    if INDEX_DF is not None:
        idx_momentum = INDEX_DF['close'].pct_change(5).to_frame('Index_Momentum')
        df = df.join(idx_momentum, how='left')
    ...
    df['Is_Safe'] = np.where(future_return > (0.25 * df['ATR']), 1, 0)
