...
LOCAL_TICK_FOLDER = os.path.join(os.path.dirname(__file__), "data", "tick_data")
...
def sync_data_fast():
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as z:
            ...
            # Find files that live inside the 'data' folder (CSV only, not tick_data subfolder)
            data_files = [f for f in all_files if '/data/' in f and '/tick_data/' not in f and f.endswith('.parquet')]

            # Find tick data parquet files
            tick_files = [f for f in all_files if '/data/tick_data/' in f and f.endswith('.parquet')]
            ...
            if not data_files and not vnindex_file and not tudoanh_files and not putthrough_files and not tick_files:
                print("⚠️ No data files found in the archive.")
                return

            print(f"📂 Found {len(data_files)} CSV files in data folder.")
            print(f"📂 Found {len(tick_files)} parquet files in data/tick_data folder.")
            ...
            # Extract tick data parquet files
            if tick_files:
                if not os.path.exists(LOCAL_TICK_FOLDER):
                    os.makedirs(LOCAL_TICK_FOLDER)
                for file_in_zip in tick_files:
                    filename = os.path.basename(file_in_zip)
                    target_path = os.path.join(LOCAL_TICK_FOLDER, filename)
                    with z.open(file_in_zip) as source, open(target_path, "wb") as target:
                        shutil.copyfileobj(source, target)
                    count += 1
                print(f"   ✅ Updated {len(tick_files)} tick data parquet files")
