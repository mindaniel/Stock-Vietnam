...
if not os.path.exists(DATA_DIR):
    ...
    st.sidebar.write(f"📊 Found {len(data_files)} parquet files in Data directory")
    if len(data_files) > 0:
...
def list_symbol_files(data_dir=DATA_DIR):
    """Liệt kê danh sách file parquet trong thư mục dữ liệu."""
